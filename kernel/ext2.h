...
private:
    ...
    uint32_t cur_sym_length; // Most recent symbol chain length
public:
    ...
    uint32_t get_sym_length() {
        auto temp = cur_sym_length;
        cur_sym_length = 0;
        return temp;
    }
        ...
        while (true) {
            if (current != nullptr) {
                if (current->is_symlink()) {
                    cur_sym_length++;
                    if (cur_sym_length >= 10) return Shared<Node>{};
                    char* sym_path = new char[current->size_in_bytes() + 1];
                    sym_path[current->size_in_bytes()] = 0;
                    current->get_symbol(sym_path);
                    current = find(root, sym_path);
                    delete[] sym_path;
                }
            }
            while (path[idx] == '/') idx++;
